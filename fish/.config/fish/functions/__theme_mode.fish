function __theme_mode -d 'Sets the theme to light or dark depending on interface style'
    switch (defaults read -g AppleInterfaceStyle 2>/dev/null)
        case Dark
            # # Fleet Dark
            # sed -i '' "s/theme =.*/theme = 'fleet_dark'/" ~/.config/helix/config.toml
            # set -Ux FZF_DEFAULT_OPTS "
            # 	--color=fg:#FFFFFF,bg:#181818,hl:#7DBEFF
            #     --color=fg+:#FFFFFF,bg+:#1F3661,hl+:#7DBEFF
            # 	--color=border:#292929,header:#FFFFFF,gutter:#181818
            # 	--color=spinner:#FFFFFF,info:#7DBEFF,separator:#FFFFFF
            # 	--color=pointer:#FFFFFF,marker:#7DBEFF,prompt:#7DBEFF"
            # command kitty +kitten themes --reload-in=parent Fleet Dark
